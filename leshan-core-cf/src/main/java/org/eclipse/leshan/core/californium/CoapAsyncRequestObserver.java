...
public class CoapAsyncRequestObserver extends AbstractRequestObserver {
    ...
    private AtomicBoolean evenRaised = new AtomicBoolean(false);
    ...
    public void onResponse(Response coapResponse) {
        ...
        if (evenRaised.compareAndSet(false, true)) {
            cleaningTask.cancel(false);
            responseCallback.onResponse(coapResponse);
        } else {
            LOG.warn("OnResponse callback ignored because an event was already raised for this request {}",
                    coapRequest);
        }
    }
    ...
    public void onTimeout() {
        ...
        if (evenRaised.compareAndSet(false, true)) {
            ...
            LOG.warn("OnTimeout callback ignored because an event was already raised for this request {}", coapRequest);
        }
    }
    ...
    public void onReject() {
        ...
        if (evenRaised.compareAndSet(false, true)) {
            ...
            LOG.warn("OnReject callback ignored because an event was already raised for this request {}", coapRequest);
        }
    }
    ...
    public void onSendError(Throwable error) {
        ...
        if (evenRaised.compareAndSet(false, true)) {
            ...
            LOG.warn("onSendError callback ignored because an event was already raised for this request {}",
                    coapRequest);
        }
    }
